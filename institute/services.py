...
def get_insitute(owner):
    ...
    if (institute_list.exists()):
        return {"exist": True, "data": institute_list.first()}

    return {"exist": False, "data": institute_list}
...
def has_subject_perm(subject, teacher):
    ...
    if (subject_accs["success"]):
        subjects = [
            subject_acc.subject for subject_acc in subject_accs['data']
        ]
...
def get_teacher_requests(institute):
    ...
    teacher_requests = TeacherRequest.objects.filter(institute=institute,
                                                     approved=False)
...
def assign_subjects(teacher, subjects, grades, institute):
    ...
    for grade in grades:
        ...
        for subject_name in subjects:
            ...
            if (subject["exist"]):
                ...
                subject_acc = SubjectAccSerializer(data={
                    "subject": subject.id,
                    "grade": grade
                })
    ...
    prev_accs = SubjectAccess.objects.filter(teacher=teacher,
                                             subject__institute=institute)
    prev_accs.delete()
    ...
    for grade in grades:
        for subject_name in subjects:
            ...
            subject_data = SubjectAccessSerializer(subject_acc,
                                                   many=False).data
...
def get_teachers_data(institute):
    ...
    teacher_reqs = TeacherRequest.objects.filter(approved=True,
                                                 institute=institute)

    teachers = [
        request_dict["teacher"]
